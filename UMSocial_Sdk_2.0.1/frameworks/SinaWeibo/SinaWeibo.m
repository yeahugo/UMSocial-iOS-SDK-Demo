...
//    [super dealloc];
    ...
    SAFE_ARC_RELEASE(userID);
    SAFE_ARC_RELEASE(accessToken);
    SAFE_ARC_RELEASE(expirationDate);
    SAFE_ARC_RELEASE(appKey);
    SAFE_ARC_RELEASE(appSecret);
    SAFE_ARC_RELEASE(appRedirectURI);
    SAFE_ARC_RELEASE(ssoCallbackScheme);
...
- (void)logIn
{
    ...
    else
    {
        ...
        if (!ssoLoggingIn) {
            [self.delegate sinaweibo:self logInDidFailWithError:nil];
        }
    }
}
